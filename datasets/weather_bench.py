...
class WeatherDataset:
    ...
    SURFACE_VARIABLE = ['2m_temperature', '10m_u_component_of_wind', '10m_v_component_of_wind', 'mean_sea_level_pressure', 'total_cloud_cover']

    VARIABLES = AIR_VARIABLE + SURFACE_VARIABLE

    DATE_OFFSET = [(2021, 2016), (2018, 2017), (2011, 2006), (2006, 2001)]
    ...
    def load_variable_optimized(self, data: xr.DataArray):
        ...
        if len(source.shape) == 4:
            ...
            inputs = torch.stack(inputs, dim=0)  # dim=0 대신 dim=1을 사용하여 stack
            return inputs.flatten(2), source.permute(1, 0, 2, 3).reshape(source.size(1), -1), stats
