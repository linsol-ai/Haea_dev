...
class WeatherDataset:
    ...
    def load_variable(self, data: xr.DataArray, key, lat_indices = None, lon_indices = None):
        ...
        if len(source.shape) == 4:
            ...
            return torch.stack(inputs, dim=0).unsqueeze(2), target.swapaxes(0, 1).unsqueeze(2), torch.tensor(means), torch.tensor(stds)
            ...
            return input.unsqueeze(1), target.unsqueeze(1), torch.tensor(mean, std
