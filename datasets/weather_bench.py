...
class WeatherDataset:
    ...
    def load_variable_1D(self, data: xr.DataArray, key, lat_indices = None, lon_indices = None):
        source = data.to_numpy()
        source = torch.from_numpy(source)
        # data.shape = (time, width, height)
        # or data.shape = (time, level, width, height)
        ...
        if len(source.shape) == 4:
            ...
            return torch.stack(inputs, dim=0).flatten, target.swapaxes(0, 1).flatten(2), torch.tensor([means, stds])
