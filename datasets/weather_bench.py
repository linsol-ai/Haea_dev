...
def preprocess_wind_data(u, v, device):
        ...
        return torch.stack([wind_speed, sin_encoded, cos_encoded], dim=0), torch.stack([un_wind_speed, un_sin_encoded, un_cos_encoded], dim=0)
    ...
    def load_data(self, dataset:xr.Dataset) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for future in tqdm(as_completed(futures), desc="Processing futures"):
                ...
                if len(normalized.shape) == 4:
                    input = input.view(-1, input.size(2), normalized.size(3))
                    unnormalized = unnormalized.view(-1, unnormalized.size(2), unnormalized.size(3))

                wind_result[level] = (normalized, unnormalized)
        ...
        for val in (self.HAS_LEVEL_VARIABLE + self.NONE_LEVEL_VARIABLE):
            ...
            if len(normalized.shape) == 3:
                ...
                unnormalized_dataset.append(unnormalized)

        normalized_dataset = torch.stack(normalized_dataset, dim=0)
        # dataset.shape => (time, var, h * w)
        normalized_dataset = torch.swapaxes(normalized_dataset, 0, 1)
