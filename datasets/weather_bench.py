...
def download_zarr(source, output_path, variables=None, levels=None):
...
class VariableVocab:
    ...
    def get_location(self, latitude, longitude):
        lat_indices = np.where((self.dataset.latitude >= latitude[0]) & (self.dataset.latitude <= latitude[1]))[0]
        lon_indices = np.where((self.dataset.longitude >= longitude[0]) & (self.dataset.longitude <= longitude[1]))[0]
...
class WeatherDataset:
    ...
    def __init__(self, start_date: datetime, end_date: datetime, device: torch.device, download_variables=None, download_levels=None):
    ...
    def check_dataset(self, start_date: datetime, end_date: datetime, download_variables, download_levels):
        idx = -1
        ...
        for resol in self.RESOLUTION:
            ...
            if not file_path.exists():
                ...
                download_zarr(gcs_path, file_path, download_variables, download_levels)
    ...
    def load_data(self, dataset:xr.Dataset, variables, constant_variables) -> Tuple[torch.Tensor, torch.Tensor]:
        start = time.time()
        result = {}
