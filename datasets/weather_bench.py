...
class WeatherDataset:
    ...
    def load_variable_optimized(self, data: xr.DataArray):
        ...
        if len(source.shape) == 4:
            ...
            return source.permute(1, 0, 2, 3).view(source.size(1), source.size(0), -1), stats

        if len(source.shape) == 2:
            source, mean, std = normalize_tensor(source)
            return source.view(-1), torch.tensor([mean, std])
    ...
    def load_data_single(self, dataset: xr.Dataset, variables, constant_variables):
        start = time.time()
        result = [self.load_variable_optimized(dataset[val]) for val in tqdm()]
