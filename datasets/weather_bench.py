...
def download_zarr(source, output_path):
    ...
    with beam.Pipeline() as root :
        (
            root
            | "Read from Source Dataset" >> xb.DatasetToChunks(source_dataset, output_chunks)
            | "Write to Zarr" >> xb.ChunksToZarr(output_path, template, output_chunks)
        )
...
class WeatherDataset:
    ...
    NONE_LEVEL_VARIABLE = ['2m_temperature', '10m_u_component_of_wind', '10m_v_component_of_wind', 'mean_sea_level_pressure', 'total_cloud_cover', 'total_precipitation_6hr']
    ...
    def __init__(self, year_offset: int, mode: int, device: torch.device, normalize=True, offline = True):
    ...
    def load_variable(self, data, key):
        data = data.to_numpy()
    ...
    def load(self):
        ...
        return input_dataset, target_dataset, normalizaion_data, dims
...
if __name__ == '__main__':
    ...
    weather = WeatherDataset(0, mode=1, device=device, offline=True)
    weather.load()