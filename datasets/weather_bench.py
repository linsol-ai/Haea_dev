...
class WeatherDataset:
    ...
    def load_data(self, dataset:xr.Dataset, wind_batch=256) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            key = executor.submit(self.calculate_wind, v1[0], v1[1], wind_batch, self.device)
            futures[key] = 0

            v2 = result[self.HAS_LEVEL_WIND_VAR]
            key = executor.submit(self.calculate_wind, v2[0], v2[1], wind_batch, self.device)
            futures[key] = 1

            
            u_wind, v_wind = result[self.v1 = result[self.HAS_LEVEL_WIND_VAR]
            key = executor.submit(self.calculate_wind, v1[0], v1[1], wind_batch, self.device)
            futures[key] = 0] key = executor.submit(self.calculate_wind, u_wind, v_wind, wind_batch, self.device)
            futures[key] = -1
