...
def preprocess_wind_data(u, v, device):
        ...
        return torch.cat([wind_speed, sin_encoded, cos_encoded], dim=0), torch.stack([un_wind_speed, un_sin_encoded, un_cos_encoded], dim=0), (wind_min, sin_min, cos_min), (wind_max, sin_max, cos_max)
...
class WeatherDataset:
    ...
    NONE_LEVEL_VARIABLE = ['toa_incident_solar_radiation', '2m_temperature', '10m_u_component_of_wind', '10m_v_component_of_wind', 'mean_sea_level_pressure', 'total_cloud_cover', 'total_precipitation']
    ...
    def load_data(self, dataset:xr.Dataset, variables, latitude: Tuple | None = None, longitude: Tuple | None = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for val in variables:
                key = executor.submit(self.load_variable, dataset[val], val, lat_indices, lon_indices)
                futures[key] = val
...
if __name__ == '__main__':
    ...
    lat = (32, 39.3)
    lon = (124, 131.4)
    weather.load(['geopotential'], lat, lon), 