...
def rekey_chunk_on_month_hour(
    key: xbeam.Key, dataset: xarray.Dataset, level=None, start_date=None, end_date=None, lat_indices=None, lon_indices=None
) -> Tuple[xbeam.Key, xarray.Dataset]:
  """Replace the 'time' dimension with 'month'/'hour'."""
  new_dataset = dataset.sel(time=slice(start_date, end_date)).sel(level=level).isel(latitude=lat_indices, longitude=lon_indices).sortby('latitude', ascending=True)
...
def main(argv):
  ...
  with beam.Pipeline(options=pipeline_options) as root :
    (
        ...
        | beam.MapTuple(rekey_chunk_on_month_hour, level=LEVEL, start_date=start_date, end_date=end_date,lat_indices=lat_indices, lon_indices=lon_indices)
