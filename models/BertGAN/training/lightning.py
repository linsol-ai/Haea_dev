...
class Embedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, variable_seq: torch.Tensor, pos_emb: torch.Tensor, lead_time_seq = None) -> torch.Tensor:
...
class BertGAN(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        d_opt = torch.optim.Adam(self.discriminator.parameters(), lr=self.config.d_lr)

        self.g_scheduler = CosineWarmupScheduler(
            g_opt, warmup=self.config.g_warmup_step, max_iters=self.max_iters
        )

        self.d_scheduler = CosineWarmupScheduler(
            d_opt, warmup=self.config.d_warmup_step, max_iters=self.max_iters
        )
    ...
    def _step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], mode: str):
        ...
        tgt_real = (self.embedding(label, var_seq, pe)) * math.sqrt(self.generator.in_dim)
        tgt_fake = (self.embedding(fake, var_seq, pe)) * math.sqrt(self.generator.in_dim)
