...
class BertGAN(pl.LightningModule):
    ...
    def _step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], mode: str) -> torch.Tensor:
        ...
        tgt = (self.embedding(label, var_seq, pe) + time_emb) * math.sqrt(self.generator.in_dim)
        tgt = (self.embedding(fake, var_seq, pe) + self.time_emb(lead_time)) * math.sqrt(self.generator.in_dim)


        real_label = torch.ones((tgt.size(0), tgt.size(1)), device=self.device)
        fake_label = torch.zeros((tgt.size(0), tgt.size(1)), device=self.device)
