...
class BertGAN(pl.LightningModule):
    ...
    def __init__(self, *, generator: Generator, discriminator: Discriminator, var_list: torch.Tensor,
                 mean_std: torch.Tensor, max_iters: int, config: TrainingConfig, max_var_len=300):
        ...
        self.embedding = Embedding(generator.max_var_len, generator.in_dim, generator.dropout)
    ...
    def _step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], mode: str) -> torch.Tensor:
        ...
        x = self.embedding(src, var_seq, lead_time, src_pe) * math.sqrt(self.generator.in_dim)
