...
class CliBERTPM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, lead_time: torch.Tensor, var_list: torch.Tensor):
        ...
        var_seq = var_list.repeat_interleave(x.size(1), dim=0).unsqueeze(0).repeat_interleave(src.size(0), dim=0)
        src_pe = positional_encoding(x.shape, x.device)
        x = x.view(x.size(0), -1, x.size(-1))
        lead_time = lead_time.unsqueeze(1).repeat(1, x.size(1))

        x = (self.embedding(x, var_seq, src_pe) + self.time_emb(lead_time)) * math.sqrt(self.in_dim)
        x = self.encoder(x)
