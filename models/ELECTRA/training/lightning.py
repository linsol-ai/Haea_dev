...
from models.ELECTRA.model.model import Electra
...
class PretrainModule(pl.LightningModule):
    ...
    def __init__(self, *, model: Electra, mean_std: torch.Tensor, var_list: torch.Tensor,
    ...
    def _step(self, batch: Dict, mode: str) -> torch.Tensor:
        ...
        label = batch['target']
        loss, mlm_loss, disc_loss = self.model(src, label, self.var_list, src_id)
        
        self.log(f"{mode}/loss", loss, prog_bar=mode == "train")
        self.log(f"{mode}/mlm_loss", mlm_loss, prog_bar=mode == "train")
