    ...
    def get_data(self, indicate):
        ...
        for t in indicate:
            ...
            if t == self.SPECIAL_TOKEN_MASK:
                data = torch.zeros(1, self.dataset.size(2), self.dataset.size(3))
            else:
...
class DenoisingDataset(Dataset):
    ...
    def make_dataset(self, size, length, mask_idx):
        ...
        for i in range(1, size-length+1):
            start = i + mask_idx + 1
            end = i + length + mask_idx + 1
    ...
    def __getitem__(self, index):
        ...
        target = torch.cat([self.vocab.dataset[target[0]-1], target_dataset], dim=0)
