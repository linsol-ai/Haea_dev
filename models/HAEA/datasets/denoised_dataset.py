...
class TimeVocab:
    ...
    def __init__(self, dataset: torch.Tensor, 
        ...
        self.max_len = self.get_max_len(bucket_size, max_len)
        self.pad_len = self.max_len - max_len 
...
class DenoisingDataset(Dataset):
    ...
    def make_dataset(self, size, pad_len, length):
        ...
        for i in range(len(self.vocab.SPECIAL_TOKENS), size-length+2):
            ...
            data = [self.vocab.SPECIAL_TOKEN_BOS] + [x for x in range(start, end)] + [self.vocab.SPECIAL_TOKEN_EOS] + [x for x in range(start, end)]
            dataset.append(data)
