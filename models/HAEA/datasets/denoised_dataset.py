...
class BARTDenoisingDataset(Dataset):
    def __init__(
        ...
        self.dataset = self.make_dataset(self.vocab.source_dataset.size(0), length)
    ...
    def make_dataset(self, size, length):
        ...
        for i in range(len(self.vocab.SPECIAL_TOKENS), size):
            ...
            dataset[i] = torch.cat([self.vocab.SPECIAL_TOKEN_BOS + torch.arange(start, end, step=1) + self.vocab.SPECIAL_TOKEN_EOS] , dim=0)
