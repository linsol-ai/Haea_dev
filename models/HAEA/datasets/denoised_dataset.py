...
class TimeVocab:
    ...
    def __init__(self, dataset: torch.Tensor, 
        ...
        self.src_max_len, self.src_pad = self.get_max_len(bucket_size, len(src_var_list), time_len)
        self.tgt_max_len, self.tgt_pad = self.get_max_len(bucket_size, len(tgt_var_list), time_len)
        self.pe = self.positional_encoding(dataset.size(-1), time_len + 10)
        self.tgt_mask = self.get_tgt_mask(len(tgt_var_list), self.tgt_max_len, self.tgt_pad)
        self.mask = self.get_mask(len(src_var_list), self.src_max_len, self.src_max_len)
    

    def get_max_len(self, bucket_size: int, var_len: int, time_len: int):
        max_len = (var_len * time_len) + 2
        if max_len % bucket_size == 0:
            return max_len, 0
        else:
            r = max_len % bucket_size
            return max_len + (bucket_size - r) , (bucket_size - r)
    ...
    def get_data(self, indicate, source: bool = True):
        ...
        padding = padding + self.pe[len(indicate)].unsqueeze(dim=0).repeat_interleave(padding.size(0), dim=0)
    ...
    def get_tgt_mask(self, var_len, max_len, pad_len) -> torch.Tensor:
        matrix = torch.zeros(max_len, max_len, dtype=torch.bool)
        matrix[:max_len-pad_len, 0] = True

        for i in range(max_len-pad_len-2):
            s =  (i * var_len) + 1
            e =  ((i+1) * var_len) + 1
            matrix[s:e, :e] = True

        matrix[max_len-pad_len+1, :max_len-pad_len+2] = True
        return matrix
