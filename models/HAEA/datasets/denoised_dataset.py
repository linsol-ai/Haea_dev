...
class HaeaVocab:
    ...
    def __init__(self, source_dataset: torch.Tensor, label_dataset: torch.Tensor, max_len, n_only_input: int = 0):
        # dataset.shape = (time, var_len, hidden)
        self.source_dataset = source_dataset
        self.label_dataset = label_dataset
        self.n_only_input = n_only_input
        self.pe = self.positional_encoding(source_dataset.size(-1), max_len)
        self.max_len = max_len * source_dataset.size(1)
    ...
    def get_data(self, indicate, dataset: torch.Tensor, source: bool = True):
        ...
        for i, t in enumerate(indicate):
            ...
            if t in self.SPECIAL_TOKENS:
                ...
                if not source:
                    var_len = dataset.size(1) - 
                    data = dataset[pos, :-self.n_only_input, :] + self.pe[i].repeat_interleave(dataset.size(1), dim=0)
