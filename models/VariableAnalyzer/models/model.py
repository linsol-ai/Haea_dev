...
class VariableAnalyzer(nn.Module):
    def __init__(self, var_len, time_len, dim_model, predict_dim, num_heads=12, n_encoder_layers=3, n_decoder_layers=3, dropout=0.1):
        super().__init__()
    ...
    def forward(self, src: torch.Tensor, tgt: torch.Tensor):
        ...
        var_seq = torch.tensor([self.var_seq for _ in range(src.size(0))], device=src.de)
    ...
    def get_time_seq(self, src: torch.Tensor):
        time_seq = torch.zeros(src.size(0), self.time_len*self.var_len)
        time_seq[:] = torch.cat([torch.full((self.var_len,), i) for i in range(0, self.time_len)])
        return time_seq
