...
class VariableAnalyzer(nn.Module):
    def __init__(self, var_len, src_time_len, tgt_time_len, dim_model, predict_dim, batch_size, num_heads=12, n_encoder_layers=3, n_decoder_layers=3, dropout=0.1):
        super().__init__()
    ...
    def get_tgt_mask(self) -> torch.tensor:
        ...
        for i in range(0, self.var_len * self.tgt_time_len):
            matrix[i, min(self.var_len*(i+1), self.var_len * self.tgt_time_len):] = float('-inf')
        return matrix
    ...
    def get_attention_maps(self, x: torch.Tensor):
        ...
        src = self.src_embedding(x, self.src_time_seq, self.src_var_seq) * math.sqrt(self.dim_model)
