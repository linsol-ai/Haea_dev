...
class Embedding(nn.Module):
    ...
    def forward(self, position_seq, variable_seq):
        x = self.position(position_seq) + self.variable(variable_seq)
        return self.dropout(x)


class VariableAnalyzer(nn.Module):
    def __init__(self, var_seq, var_len, time_len, dim_model, num_heads=12, n_encoder_layers=3, n_decoder_layers=3, dropout=0.1):
        super().__init__()
        self.var_seq = var
    ...
    def forward(self, src: torch.Tensor, tgt: torch.Tensor):
        ...
        tgt = self.tgt_embedding(time_seq) * math.sqrt(self.dim_model)
