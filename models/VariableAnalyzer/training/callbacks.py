...
class VariableVaildationCallback(Callback):
    ...
    def __init__(
        self, level_var: List, non_level_var: List, val_batch: int, log_every_n_step: int, dataset: Dataset, logger: WandbLogger
    ) -> None:
        """Init the callback.
    ...
    def validation(self, pl_module: TrainModule):
        ...
        tgt = torch.stack([self._dataset[i][0] for i in range(self.val_batch)], dim=0).to(
            pl_module.device  # type: ignore[arg-type]
        )
