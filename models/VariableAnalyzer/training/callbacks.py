...
class VariableVaildationCallback(Callback):
    ...
    def __init__(
        self, level_var: List, non_level_var: List, level_info: List, log_batch: int, log_every_n_step: int, dataset: Dataset, logger: WandbLogger
    ) -> None:
        """Init the callback.
    ...
    def validation(self, pl_module: TrainModule):
        ...
        tgt = torch.stack([self._dataset[i][1] for i in range(self.log_batch)], dim=0).to(
            pl_module.device  # type: ignore[arg-type]
        )
        label = torch.stack([self._dataset[i][2] for i in range(self.log_batch)], dim=0).to(
            pl_module.device  # type: ignore[arg-type]
        )
        predict = pl_module.model(src, tgt)
        loss = pl_module.calculate_loss(predict[:, :, :pl_module.predict_dim], label[:, :, :pl_module.predict_dim])
        ...
        pl_module.model.init_seq(self.log_batch, pl_module.device)