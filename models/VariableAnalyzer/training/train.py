...
def _main(args) -> None:
    ...
    try:
        ...
        train_loader = DataLoader(
            train_ds, batch_size=config.training.batch_size, shuffle=True, drop_last=True, num_workers=8
        )
        ...
        model_pl = TrainModule(model=model, mean_std=mean_std, var_len=shape[1], predict_dim=dims[0], max_iters=config.training.max_epochs*len(train_loader), config=config.training)
