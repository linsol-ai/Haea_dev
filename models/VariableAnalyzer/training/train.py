...
FLAGS = flags.FLAGS
YEAR_OFFSET = flags.DEFINE_multi_integer('train_offset', None, help='training year')
TIME_LENGTH = flags.DEFINE_multi_integer('time_len', None, help='TIME_LENGTH')
...
def _main(args) -> None:
    config_path = os.path.join(os.path.dirname(os.path.abspath(os.path.dirname(__file__))), 'configs/train_set.yaml')
    try:
        ...
        dataset, shape = get_dataset(train_offset, time_len)
