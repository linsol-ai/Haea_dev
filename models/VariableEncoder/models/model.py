...
class VariableEncoder(nn.Module):
    ...
    def positional_encoding(self, shape, device):
        pe = torch.zeros(shape).float()
        pe.require_grad = False

        position = torch.arange(0, shape[1]).float().unsqueeze(1)
        div_term = (torch.arange(0, shape[3], 2).float() * -(math.log(10000.0) / shape[3])).exp()

        pe[:, :, 0::2] = torch.sin(position * div_term)
        pe[:, :, 1::2] = torch.cos(position * div_term)

        return pe.repeat_interleave(shape[2], dim=1).to(device)
