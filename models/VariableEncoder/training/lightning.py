...
def positional_encoding(d_model, var_len, time_len):
    pe = torch.zeros(time_len, d_model).float()
...
class TrainModule(pl.LightningModule):
    ...
    def __init__(self, *, model: VariableEncoder, mean_std: torch.Tensor, src_var_list: torch.Tensor, tgt_var_list: torch.Tensor,
        ...
        self.pe = positional_encoding(model.in_dim, tgt_var_list.size(0), config.tgt_time_len + 10)
