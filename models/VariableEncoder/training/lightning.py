...
class TrainModule(pl.LightningModule):
    ...
    def visualization_level(self, level_loss: torch.Tensor):
        for i, name in range(len(self.config.atmospheric_variable)):
    ...
    def validation(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]):
        ...
        level_loss = loss[:self.levels * len(self.config.atmospheric_variable), :]
        non_level_loss = loss[self.levels * len(self.config.atmospheric_variable):, :]
