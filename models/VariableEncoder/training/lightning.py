...
class TrainModule(pl.LightningModule):
    ...
    def calculate_rmse_loss(self, predict: torch.Tensor, target: torch.Tensor):
        ...
        label = reversed_predict.view(reversed_predict.size(0), -1, reversed_predict.size(3))

        # predict.shape = (batch, time_len * var_len, hidden) -> not nomalized
        predict = predict.view(predict.size(0), -1, target.size(2), predict.size(2))
    ...
    def validation(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]):
        ...
        air_loss = loss[:self.config.pressure_level * len(self.config.air_variable), :]
        surface_loss = loss[self.config.pressure_level * len(self.config.air_variable):, :]
