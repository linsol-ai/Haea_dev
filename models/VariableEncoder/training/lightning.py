...
def get_var_seq(src_var_list: torch.Tensor, tgt_var_list: torch.Tensor, src_time_len: int, tgt_time_len: int, batch_size: int):
    ...
    tgt_seq = torch.cat([SPECIAL_TOKEN_BOS, tgt_seq], dim=0)
...
class TrainModule(pl.LightningModule):
    ...
    def _step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], mode: str) -> torch.Tensor:
        ...
        label = batch[1]
        zero_tensor = torch.zeros(label.size(0), 1, label.size(2))
        tgt = torch.cat([zero_tensor, label], dim=1)
        src = src + positional_encoding(src.size(0), self.config.src_time_len, self.src_var_list.size(0), src.size(-1))
        tgt = tgt + positional_encoding(tgt.size(0), self.config.tgt_time_len, self.src_var_list.size(0), src.size(-1))
