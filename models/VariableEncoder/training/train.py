...
def get_normal_dataset(config: TrainingConfig, year_offset: int, tgt_time_len: int):
    ...
    source, label, mean_std = weather.load(config.air_variable, config.surface_variable, config.only_input_variable)
    dataset = CustomDataset(source, label, tgt_time_len, n_only_input=len(config.only_input_variable))
    src_var_list = weather.get_var_code(config.air_variable + config.surface_variable + config.only_input_variable)
    tgt_var_list = weather.get_var_code(config.air_variable + config.surface_variable + config.only_input_variable)
    return dataset, source.shape, mean_std, 
...
def _main(args) -> None:
    ...
    try:
        ...
        dataset, shape, mean_std, out_dim = get_normal_dataset(config.training, train_offset, tgt_time_len)
        ...
        train_loader = DataLoader(
            train_ds, batch_size=config.training.batch_size, shuffle=True, drop_last=True, num_workers=2
        )
