...
from models.dVAE.predict import VariableProprecessor
...
def get_normal_dataset(year_offset: int, tgt_time_len: int):
    device = ("cuda" if torch.cuda.is_available() else "cpu" )
    device = torch.device(device)

    weather = WeatherDataset(year_offset, device=device)
    # dataset.shape:  torch.Size([7309, 100, 1450])
    source, label, mean_std = weather.load(weather.HAS_LEVEL_VARIABLE + weather.NONE_LEVEL_VARIABLE)
...
def _main(args) -> None:
    ...
    try:
        ...
        val_loader = DataLoader(val_ds, batch_size=config.training.batch_size, drop_last=True, num_workers=)
