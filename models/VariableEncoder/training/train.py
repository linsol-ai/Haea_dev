...
def get_dataset(year_offset: int, tgt_time_len: int, latitude, lon):
    device = ("cuda" if torch.cuda.is_available() else "cpu" )
    device = torch.device(device)

    weather = WeatherDataset(year_offset, mode= WeatherDataset.RESOLUTION_MODE_BASIC_SET, device=device)
    # dataset.shape:  torch.Size([7309, 100, 1450])
    input, target, mean_std, dims = weather.load(weather.HAS_LEVEL_VARIABLE + weather.NONE_LEVEL_VARIABLE, latitude=lat, longitude=lon)
