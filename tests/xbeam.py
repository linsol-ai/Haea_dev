...
def main(argv):
  ...
  lat_indices = np.where((source_dataset.latitude >= lat_min) & (ds.latitude <= lat_max))[0]
  lon_indices = np.where((source_dataset.longitude >= lon_min) & (ds.longitude <= lon_max))[0]
